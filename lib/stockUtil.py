...
def searchStockNameFromCSV(marketType,queryString):
  ...
  if marketType=="TW":
    ...
    found=re.findall(regStr, text, re.M)
    #print(found)
    if found!=None:
      for a in found:
        ary.append(a[1])
  ...
  for row in reader:
    if queryString1 in row[0]:
      ary.append(row[0])
    if queryString1 in row[1]:
      ary.append(row[1])
  f1.close()
  return ary
...
def getStockNameFromCSV(marketType, stockId): #return None if stockId not found
  ...
  if marketType=="TW": #找上市公司名單
    text = open(PROJECT_ROOT + "/lib/台股上市公司20160324.csv").read()
    #print(text)
    found=re.search(regStr,text,re.M);
    if found!=None: 
      #print(found.group())
...
def getMarketId(marketType, stockId):
  if marketType=="TW":  #台股上市或上櫃
    text = open(PROJECT_ROOT + "/lib/台股上市公司20160324.csv").read()
    if re.search(stockId, text, re.M) != None:  return marketType
    else:  return 'TWO'
  if marketType=="US":
    str= r'"' + stockId +r'"' 
    #print(str)
    text = open(PROJECT_ROOT + "/lib/NYSEcompanylist.csv").read()
    if re.search(str, text, re.M) != None:  return "NYSE"
    text = open(PROJECT_ROOT + "/lib/AMEXcompanylist.csv").read()
    if re.search(str, text, re.M) != None:  return "NYSE"  #AMEX is acquire by NASDAQ
    text = open(PROJECT_ROOT + "/lib/NASDAQcompanylist.csv").read()
    if re.search(str, text, re.M) != None:  return "NASDAQ"
    return marketType
  if marketType=="HK":
    return marketType
  return marketType
