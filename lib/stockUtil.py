...
def searchStockNameFromCSV(pfGroup,queryString):
  ...
  #print(regStr)
  if pfGroup=="TW":
    text = open(PROJECT_ROOT + "/lib/台股上市公司20171219.csv", encoding='utf8').read()
    found=re.findall(regStr, text, re.M|re.I)
    #print(found)
    if found!=None:
      for a in found:
        ary.append(a[1])
    text = open(PROJECT_ROOT + "/lib/台股上櫃公司20171219.csv", encoding='utf8').read()
    found=re.findall(regStr, text, re.M|re.I)
    #print(found)
    if found!=None:
      for a in found:
        ary.append(a[1])
  if pfGroup=="HK":
    text = open(PROJECT_ROOT + "/lib/港股代碼20160212.csv", encoding='utf8').read()
    found=re.findall(regStr, text, re.M|re.I)
    if found!=None:
      #print(found)
      for a in found:
